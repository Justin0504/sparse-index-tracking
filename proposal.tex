...
\textbf{Why differentiate through the QP rather than use surrogate losses?}\; Under the hard-$K$ formulation, stock selection is discrete and non-differentiable, but we hold the selected set $\mathcal{S}$ fixed (detached from the computational graph) and only differentiate through the continuous QP over weights.  This is sufficient because the QP already captures how covariance errors affect portfolio quality within the selected subset.  Alternative approaches such as randomized smoothing \citep{wilder2019melding} or policy-gradient surrogates could in principle make the selection step itself differentiable, and we consider these as extensions if time permits.  However, our $\ell_1$ formulation (Formulation~B) already provides a fully differentiable alternative that does not require surrogate methods, enabling a direct comparison of the two paradigms.

\textbf{Role of context features.}\; Our features (trailing volatility, momentum, sector, and market-cap quintile) serve as conditioning variables that allow the covariance prediction to adapt to changing market regimes.  In the factor model, the loading matrix $B$ is a linear function of these features, so different feature realizations produce different covariance structures.  The choice of features affects what regime variation the model can capture: for instance, removing sector features would reduce the model's ability to distinguish sector-rotation events from broad market moves.  We use the same feature set across all methods to ensure a fair comparison; exploring richer feature sets (e.g., lagged cross-sectional signals, macro indicators) is left to future work.
